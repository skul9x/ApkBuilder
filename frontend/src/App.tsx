...
import './App.css';
import LogcatViewer from './components/LogcatViewer/LogcatViewer';
// @ts-ignore
import {
    SelectFolder, 
    LoadProject, 
...
function App() {
    ...
    const [activeTab, setActiveTab] = useState<'builder' | 'logcat'>('builder');
    ...
    return (
        <div className="flex flex-col h-screen bg-[#060b14] text-[#f8fafc] font-sans select-none overflow-hidden">
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-[340px] bg-[#0b1120] border-r border-[#1e293b] p-8 flex flex-col space-y-10 overflow-y-auto">
                    ...
                    <div className="flex-1 flex flex-col justify-end">
                        ...
                        <div className="mt-4 border-t border-[#1e293b] pt-4">
                            <button 
                                onClick={() => setActiveTab(activeTab === 'builder' ? 'logcat' : 'builder')}
                                className={`w-full py-4 text-[11px] uppercase tracking-widest font-bold rounded-2xl border transition-all flex items-center justify-center space-x-2 ${activeTab === 'logcat' ? 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30' : 'bg-[#1e293b] hover:bg-[#334155] text-[#94a3b8] border-[#334155]'}`}
                            >
                                ...
                                <span>{activeTab === 'builder' ? 'Mở Logcat Viewer' : 'Trở Về Builder'}</span>
                            </button>
                        </div>
                    </div>
                </aside>
                ...
                {/* Main Content Area */}
                <div className="flex-1 flex flex-col p-8 space-y-8 h-full">
                    {activeTab === 'logcat' ? (
                        <LogcatViewer />
                    ) : (
                        <>
                            ...
                            {/* Console Window */}
                            <div className="console-window flex-1 relative flex flex-col min-h-0">
