...
class AndroidBuilderApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Android Builder Pro v6.1 (Added Copy Log)")
    ...
    def setup_ui(self):
        ...
        # --- Bottom Buttons (Copy Log & Open Folder) ---
        bottom_layout = QHBoxLayout()

        # NÚT MỚI: COPY LOG
        btn_copy_log = QPushButton("📋 Copy Log")
        btn_copy_log.setMinimumHeight(40)
        btn_copy_log.setStyleSheet("background-color: #9C27B0; color: white; font-weight: bold; border-radius: 5px;")
        btn_copy_log.setCursor(Qt.PointingHandCursor)
        ...
        bottom_layout.addWidget(btn_copy_log)
    ...
    def copy_log_to_clipboard(self):
        ...
        log_content = self.console_log.toPlainText()
        if log_content:
            clipboard.setText(log_content)
            # Thông báo nhỏ trong log để biết đã copy
            self.log("✅ Đã copy toàn bộ log vào Clipboard!", "success")
            # Hoặc hiện popup nếu muốn
            # QMessageBox.information(self, "Copy", "Đã copy log thành công!")
        else:
            QMessageBox.warning(self, "Copy", "Log đang trống, không có gì để copy.")
    ...
    def check_adb_devices(self):
        ...
        try:
            proc = subprocess.Popen([self.adb_path, "devices"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=0x08000000 if platform.system()=='Windows' else 0)
            out, _ = proc.communicate()
            if "List of devices" in out.decode():
                c = len([l for l in out.decode().splitlines() if l.strip() and "List" not in l])
                self.lbl_device_status.setText(f"🟢 {c} Devices" if c else "🔴 No Device")
        except: pass
